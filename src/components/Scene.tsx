...
export default function Scene() {
	...
	useEffect(() => {
		...
		const obj = interactiveObjectsRef.current.find((o: any) => o.key === focusKey)
		if (obj && (!focusedObjectRef.current || focusedObjectRef.current.key !== focusKey))
	...
	useEffect(() => {
		...
		const isMobile =
			window.innerWidth <= 600 || ('ontouchstart' in window && window.innerWidth <= 1024)

		// Renderer
		const renderer = new THREE.WebGLRenderer({ canvas: canvasEl, antialias: !isMobile })
		...
		scene.fog = new THREE.FogExp2(0xd0ccc8, 0.010)
		...
		const { hourPivot, minPivot, secPivot } = buildClockMesh(scene, clockGLTF)
		...
		function getAllMeshes(obj: THREE.Object3D) {
			...
			obj.traverse((c: any) => { if (c.isMesh) out.push(c) })
			return out
		}
			...
			updateRoomLighting(
				{
					...
					laptopScreenMats,
					lampOn,
					onDeskGlowBase: (v) => { deskGlowBase.value = v },
					onLampLightBase: (v) => { lampLightBaseRef.current = v },
				},
				now
			)
		}
		...
		function doResetCamera() {
			...
			setFocus(null)
			closePanel()
			animateCamera(DEFAULT_CAM_POS, DEFAULT_CAM_TARGET, () => {
		...
		function toggleLampState() {
			...
			if (lampOn) {
				...
				;(lampBulbRef.current.material as THREE.MeshStandardMaterial).emissiveIntensity = 4.0
			} else {
				...
				;(lampBulbRef.current.material as THREE.MeshStandardMaterial).emissiveIntensity = 0
			}
		}
		...
		canvasEl.addEventListener('mousemove', (e: MouseEvent) => {
			...
			const obj = hits.length > 0 ? meshToObject.get(hits[0].object) ?? null : null
			if (obj) {
		...
		function onWindowClick(e: MouseEvent) {
			...
			if (lampHits.length > 0) { toggleLampState(); return }
			const hits = raycaster.intersectObjects(allInteractiveMeshes, false)
			if (hits.length > 0) {
				const obj = meshToObject.get(hits[0].object) ?? null
				if (obj) { focusObject(obj); return }
			}
		...
		function animate(time: number) {
			...
			lights.deskGlow.intensity = deskGlowBase.value + Math.sin(time * 0.0028) * 0.06
			if (lampOn)
			...
			for (let i = 0; i < PARTICLE_COUNT; i++) {
				...
				posAttr.setX(i, posAttr.getX(i) + Math.sin(time * 0.0004 + i) * dt * 0.010)
			}
